...
#define DEF_STAT(x) { #x, sizeof(#x) - 1, offsetof(ngx_buffer_cache_stats_t, x) }

static ngx_stat_def_t buffer_cache_stat_defs[] = {
	DEF_STAT(store_ok),
	DEF_STAT(store_bytes),
	DEF_STAT(store_err),
	DEF_STAT(fetch_hit),
	DEF_STAT(fetch_bytes),
	DEF_STAT(fetch_miss),
	DEF_STAT(evicted),
	DEF_STAT(evicted_bytes),
	DEF_STAT(reset),
	DEF_STAT(entries),
	DEF_STAT(data_size),
	{ NULL, 0, 0 }
...
static u_char*
append_cache_stats(u_char* p, ngx_buffer_cache_stats_t* stats)
{
